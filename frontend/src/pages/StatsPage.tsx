    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.todayMinutes} min</div>
            <p className="text-xs text-muted-foreground">
              Semana: {stats.weekMinutes} min · Mes: {stats.monthMinutes} min
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="flex items-center justify-between text-sm">
            <span>Intentados: {stats.attemptedPomodoros}</span>
            <span>Completados: {stats.completedPomodoros}</span>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Ritmo reciente</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            {stats.byDay.filter((day) => day.minutes > 0).length} de los ultimos 7 dias tuvieron actividad registrada.
          </CardContent>
        </Card>
      </div>
            ...
            {stats.byCourse.map((course) => (
                <div key={course.courseId} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-muted-foreground">{course.minutes} min</span>
                  </div>
